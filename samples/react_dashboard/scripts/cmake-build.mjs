...
const buildDir = path.join(sampleRoot, "build");
...
async function resolveCMakeExe() {
  ...
  if (process.platform === "win32") {
    ...
    // Common Visual Studio bundled CMake locations.
    const vsCandidates = [
  ...
  // Fall back to PATH name (may still work on non-Windows).
  return "cmake";
}

fs.mkdirSync(buildDir, { recursive: true });

// Multi-config generators (VS) will ignore CMAKE_BUILD_TYPE, but it's harmless.
const cmake = await resolveCMakeExe();

// On Windows, if CMake isn't installed or discoverable, provide a clear error.
  ...
  try {
    ...
    console.error(
      [
        ...
        "",
        "Common Visual Studio path (example):",
        "  C:\\Program Files\\Microsoft Visual Studio\\2022\\Community\\Common7\\IDE\\CommonExtensions\\Microsoft\\CMake\\CMake\\bin\\cmake.exe",
      ].join("\n")
    );
    process.exit(1);
...
await run(cmake, ["-S", sampleRoot, "-B", buildDir, "-DCMAKE_BUILD_TYPE=Release"]);
